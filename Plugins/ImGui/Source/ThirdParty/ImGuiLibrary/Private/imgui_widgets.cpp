...
bool ImGui::TreeNode(const char* label, float OverrideWidth)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    return TreeNodeBehavior(window->GetID(label), 0, label, NULL, OverrideWidth);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, float InOverrideWidth)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    frame_bb.Max.x = InOverrideWidth != 0.f ? frame_bb.Min.x + InOverrideWidth : window->WorkRect.Max.x;
